...
- `examples/nqueens.pebble` — N-Queens solution counter via backtracking
...
- `examples/dijkstra.pebble` — Dijkstra's single-source shortest-path algorithm with optional path reconstruction
...
- `examples/huffman.pebble` — Huffman coding for string compression and decompression with character frequency analysis
...
- `examples/calendar.pebble` — Proleptic Gregorian calendar library with date validation, weekday calculation, and date arithmetic
...
All examples are covered by tests in `tests/test_examples.py`, `tests/test_bst.py`, `tests/test_bfs.py`, `tests/test_dijkstra.py`, `tests/test_bank_account.py`, `tests/test_collatz.py`, `tests/test_edit_distance.py`, `tests/test_hanoi.py`, `tests/test_huffman.py`, `tests/test_json.py`, `tests/test_matrix.py`, `tests/test_rpn.py`, `tests/test_streams.py`, `tests/test_primes.py`, `tests/test_brainfuck.py`, `tests/test_heap.py`, `tests/test_metacircular.py`, `tests/test_cipher.py`, `tests/test_vigenere.py`, `tests/test_number_words.py`, `tests/test_nqueens.py`, `tests/test_queue.py`, `tests/test_rational.py`, and `tests/test_calendar.py`.
...
- `coin-change` — `(coin-change amount coins)` returns the minimum number of coins (from the list `coins` of positive integer denominations, each usable any number of times) that sum exactly to `amount`, or `-1` if `amount` cannot be made from the given coins. `(coin-change 0 coins)` returns 0. Implemented using bottom-up dynamic programming with a mutable vector for the DP table, achieving O(amount × len(coins)) time complexity. Requires `amount` to be a non-negative integer and `coins` to be a non-empty list of positive integers; raises an error otherwise. Examples: `(coin-change 11 (list 1 2 5))` → `3` (5 + 5 + 1), `(coin-change 6 (list 1 3 4))` → `2` (3 + 3), `(coin-change 3 (list 2))` → `-1` (impossible)
- `lcs-length` — `(lcs-length a b)` returns the length of the longest common subsequence (LCS) of strings `a` and `b`. A longest common subsequence is a sequence of characters that appear in both strings in the same relative order but need not be contiguous. For example, the LCS of `"ABCBDAB"` and `"BDCAB"` is `"BCAB"` with length 4. The function uses bottom-up dynamic programming with a mutable 2D DP table (represented as a vector of row-vectors) to achieve O(m × n) time complexity, where m and n are the lengths of the two strings. Examples: `(lcs-length "ABCBDAB" "BDCAB")` → `4`, `(lcs-length "AGGTAB" "GXTXAYB")` → `4`, `(lcs-length "abc" "def")` → `0`, `(lcs-length "abc" "")` → `0`
