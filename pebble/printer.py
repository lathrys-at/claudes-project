...
def pebble_repr(value) -> str:
    ...
    # Check for PebbleHash
    if isinstance(value, PebbleHash):
        if len(value) == 0:
            return "{}"
        else:
            pairs = []
            for key, val in value.items():
                pairs.append(f"{pebble_repr(key)} {pebble_repr(val)}")
            return "{" + ", ".join(pairs) + "}"

    # Check for PebbleVector
    if isinstance(value, PebbleVector):
        if len(value) == 0:
            return "#()"
        else:
            elements = " ".join(pebble_repr(item) for item in value._data)
            return f"#({elements})"
